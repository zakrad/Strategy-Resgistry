...
contract Proxy is IProxy, Storage, Config {
    ...
    /**
     * @notice Strategy Implementation that should override in Strategy Proxy contract
     * It should include three phases: pre-process, exection and post-process.
     */
    function execStrategy() external payable virtual isNotHalted isNotBanned returns (bytes memory) {
     ...
     */
    function execs(address[] calldata tos, bytes32[] calldata configs, bytes[] memory datas)
        external
        payable
        override
        isNotHalted
     ...
     */
    function _execs(address[] memory tos, bytes32[] memory configs, bytes[] memory datas) internal {
     ...
     */
    function _trim(bytes memory data, bytes32 config, bytes32[256] memory localStack, uint256 index) internal pure {
     ...
     */
    function _exec(address to_, bytes memory data_, uint256 counter_) internal returns (bytes memory result) {
